function account() {
  ...
  account.prototype.register = function(Defaults, Data) {
    ...
    connection.query(
      'UPDATE rr_users SET users_loggedin = ? WHERE users_device = ? AND users_username = ?',
      [false, Data.device, Data.username],
      function(err, results) {
        ...
        connection.query(
          'SELECT * FROM rr_users WHERE users_username = ?',
          [Data.username],
          function(err, results) {
            if (err) {
              resolve({ status: -1, text: 'MySQL errors 2', error: err });
            }
            if (results.length > 0) {
              resolve({
                status: 0,
                text: (
                  'User ' + Data.username + ' already exists in the database'
                )
              });
            } else {
              //insert user
              connection.query('INSERT INTO rr_users SET ?', insert, function(
                err,
                result
              ) {
                if (err) {
                  resolve({ status: -1, text: 'MySQL errors 3', error: err });
                }
                connection.query(
                  'SELECT * FROM rr_users WHERE users_id = ?',
                  [result.insertId],
                  function(err, results) {
                    if (err) {
                      resolve({
  ...
  account.prototype.login = function(Defaults, Data) {
    ...
    // reset all logged in where device
    connection.query(
      'SELECT * FROM rr_users WHERE users_username = ? AND users_password = ?',
      [Data.username, JSON.stringify(encrypt(Data.password))],
      function(err, results) {
        if (err) {
          resolve({ status: -1, text: 'MySQL errors 1', error: err });
        }
        if (results.length > 1) {
          ...
          connection.query(
            'UPDATE rr_users SET users_loggedin = ? WHERE users_device = ? AND users_username = ?',
            [true, Data.device, results[0].users_username],
            function(err, results2) {
              if (err) {
                resolve({ status: -1, text: 'MySQL errors 1', error: err });
              }
              connection.query(
                'UPDATE rr_users SET users_device = ? WHERE users_username = ?',
                [Data.device, results[0].users_username],
  ...
  account.prototype.logout = function(Defaults, Data) {
    ...
    // reset all logged in where device
    connection.query(
      'UPDATE rr_users SET users_loggedin = ? WHERE users_username = ?',
      [false, Data.username],
      function(err, results) {
        if (err) {
          resolve({ status: -1, text: 'MySQL errors 1', error: err });
  ...
  account.prototype.checkLogin = function(Defaults, Data) {
    ...
    connection.query(
      'SELECT * FROM rr_users WHERE users_device = ? AND users_loggedin = ? ORDER BY users_time DESC LIMIT 1',
      [Data.device, true],
      function(err, results) {
        if (err) {
          resolve({ status: -1, text: 'MySQL errors 1', error: err });
