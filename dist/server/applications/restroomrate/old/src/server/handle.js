...
function API(url, urlStructure) {
  return new Promise(function (resolve, reject) {
    switch (urlStructure[1]) {
      case 'register':
        Account.register({ onComplete: resolve, onFail: reject, connection: conn }, {
      ...
      case 'logout':
        Account.logout({ onComplete: resolve, onFail: reject, connection: conn }, { device: url.query.device, username: url.query.username });
        break;

      case 'checkLogin':
        Account.checkLogin({ onComplete: resolve, onFail: reject, connection: conn }, { device: url.query.device });
        break;

      case 'login':
        Account.login({ onComplete: resolve, onFail: reject, connection: conn }, {
          device: url.query.device,
          username: url.query.username,
          password: url.query.password
        });
      ...
      case 'delete':
        Content.delete({ onComplete: resolve, onFail: reject, connection: conn }, url.query);
        break;

      case 'report':
        Actions.report({ onComplete: resolve, onFail: reject, connection: conn }, url.query);
        break;
