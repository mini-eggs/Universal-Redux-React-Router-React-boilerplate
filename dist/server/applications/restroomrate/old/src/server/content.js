...
function content() {
  ...
  content.prototype.recent = function (Defaults, Data) {
    ...
    this.filter(connection, params).then(function (filter) {
      connection.query('SELECT * FROM rr_rate WHERE rate_id NOT IN (?) AND rate_user NOT IN (?) ORDER BY rate_time DESC LIMIT ? OFFSET ?', [filter.reportedIds, filter.reportedUsers, limit, offset], function (err, results) {
        if (err) {
  ...
  content.prototype.nearby = function (Defaults, Data) {
    ...
    this.filter(connection, params).then(function (filter) {
      connection.query('SELECT *,\n              SQRT(POW(69.1 * (rate_lat - ?), 2) + POW(69.1 * (? - rate_long) * COS(rate_lat / 57.3), 2)) AS distance\n              FROM rr_rate WHERE rate_id NOT IN (?) AND rate_user NOT IN (?) ORDER BY distance LIMIT ? OFFSET ?', [params.lat, params.long, filter.reportedIds, filter.reportedUsers, limit, offset], function (err, results) {
        if (err) {
          resolve({ status: -1, text: 'MySQL errors 1', error: err });
        }
        resolve({
  ...
  content.prototype.delete = function (Defaults, Data) {
    ...
    connection.query('DELETE FROM rr_rate WHERE rate_id = ? && rate_user = ? ', [params.rate_id, params.users_id], function (err, result) {
      if (err) {
